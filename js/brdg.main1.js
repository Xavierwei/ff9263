...
function appendLatestItems() {
    $_node.getPreNextNodes(lastNid, 20, 0, null, function (data) {
        ...
        if(!_.isEmpty(data[i]))
        {
            ...
            if (_daySpan.length > 0) {
               ...
               else
               {
                ...
                var daySpan = $('<div class="date_span"><div class="full_time">' + $_utility.formateNumber(media_date_day) + '/' + $_utility.formateNumber(media_date_month) + '/' + $_utility.formateNumber(media_date_year) + '</div><div class="day">' + $_utility.formateNumber(media_date_day) + '</div><div class="city">'+STR_CITYLABEL[citylabel]+'</div></div>').appendTo(".vgrid");
                daySpan.css({
                    'width': imageWidth,
                    'height': imageWidth
                });
    ...
    items.each(function () {
        ...
        if (!_c.hasClass('opened')) {
            ...
            itemsTimeout = setTimeout(function () {
                ...
                _c.clearQueue().delay(200).transition({
                    perspective: '500px',
                    rotateY: '0deg',
                    opacity: 1
                }, 1000, 'easeOutQuart');
...
function loadSingleContent(nid, isOpen) {
    ...
    $_node.getPreNextNodes(nid, 10, current_type, current_cityid, function (data) {
        clickable = true;
        $('.loading2').fadeOut();
...
function renderMediaContent(media_data, face, noimg) {
    ...
    if (!noimg) {
        ...
        if (media_data.media_source === 'youtube') {
            ...
            $('.dc3_wrap').eq(imgIndex).html('<iframe src="vine/vine_resize.php?q='+url[1]+'&w='+ $('.dc3_wrap').eq(0).width() +'" width="100%" height="100%" frameborder="0" scrolling="no"></iframe>');
        }
        else {
...
function bindNodeEvent() {
    ...
    // Load comments
    $.Comment(nid).loadComment(function (data) {
        var comments_wrap = $('.dc2_bigcube_face1 .comments_list').empty();
        for (var c in data) {
...
function resizeWindow() {
    ...
    perpage = parseInt(($(window).width() / imageWidth)) * Math.ceil(($(window).height() / imageWidth))*2;
