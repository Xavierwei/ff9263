...
*/
$.Node = function(){
    function getPreNextNodes(nid, count, type, cityid, cb)
    {
        $.ajax({
            url: siteurl+"data/third_content/node/pre_next_node",
            dataType: "JSON",
            type: "POST",
            data: JSON.stringify({nid:nid,number:count,resource_type:type,city:cityid}),
...
$.Comment = function(nid){
    ...
    function submitComment(form, cb, errorCb){
        ...
        var comment = {
               ...
               field_email: {und: [{value: form.find('input[name="email"]').val()}]}
        };
        $.ajax({
                url: apipath + "/comment",
                dataType: "JSON",
