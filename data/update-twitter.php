...
set_time_limit(10000 * 600); // 60 seconds
...
$sql = "SELECT * FROM `node` where title = 'from twitter'";
$query = db_query($sql);
//foreach ($query as $result) {
//    $node = nodeLoad($result->nid);
//    print_r($node);
//}

//$nodes = node_load_multiple(array('7188'), array('type' => 'content_from_source'));
//print_r($nodes);
foreach($query as $result) {
    try{
        $node = node_load($result->nid);
        //print_r($node);
        $username = $node->field_from_user_name[und][0][value];
        $drupal_account = brdg_core_load_drupal_user_with_screen_name($username, 'twitter');
        print_r($drupal_account->uid);
